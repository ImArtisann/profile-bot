	...
	async createProfilePic(guildId, user, member, test = false) {
		...
		const drawMoodBar = () => {
			try {
				let strokeWidth = 3
				const moodFillWidth =
					(((data?.mood || 4) * 10) / 100) * this.variables.width
		...
		const drawFocusBar = () => {
			try {
				...
				const focusFillWidth =
					(((data?.focus || 4) * 10) / 100) * this.variables.width
		...
		const drawManaBar = () => {
			try {
				let strokeWidth = 3
				const manaFillWidth =
					(((data?.mana || 4) * 10) / 100) * this.variables.width
		...
		const addRaidEmblems = async () => {
			let raids = fs
				.readdirSync(path.join(foldersPath, 'raids'))
